...
static void start_rx(hw_rx_cfg_t const* rx_cfg) {
  ...
  if(lora_mode) {
    ...
    if(rssi_valid_callback != 0)
    {
        ...
        // The RSSI value is smoothed on 8 RSSI samples for an estimated accuracy lower than 5db.
        // We need to wait for the RSSI response time  = 2^( RssiSmoothing + 1 ) / (4⋅RxBw [ kHz ])
        DPRINT("Wait during RSSI response time");
        hw_busy_wait(160); // minimum CCA interval 160µs
...
error_t hw_radio_init(alloc_packet_callback_t alloc_packet_cb, release_packet_callback_t release_packet_cb) {
  ...
  sched_register_task(&switch_to_standby_mode);
